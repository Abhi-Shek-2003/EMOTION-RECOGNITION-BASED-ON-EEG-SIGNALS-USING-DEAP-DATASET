...
def get_DASM_electrode_indices():
	...
	# identify the left and right electrodes for differential analysis.
...
# prints statistical information about features extracted from different frequency bands
...
def get_trial_PSD(data_epoch, duration_baseline, duration_epoch, time_range, normalize=False):
	...
	data_baseline = data_epoch[:, :duration_baseline].copy() # includes the first duration_baseline columns
	data_trial = data_epoch[:, duration_baseline:].copy() 	# data_trial includes the remaining columns.
